...
2026-09-04부터 `[[Vault Schema]]` v2를 단일 기준으로 삼는다. paper·concept·map·comparison·synthesis에는 검색과 목록 보기에 쓸 짧은 `summary`를 두고, concept는 `maturity`와 `last_reviewed`로 성숙도를 관리한다. comparison과 synthesis의 canonical home은 각각 `35 Comparisons/`, `37 Syntheses/`다. 검색은 좁은 범위에서 시작해 필요할 때만 넓히고, 수정은 문서 전체 교체보다 frontmatter나 heading 단위의 작은 patch를 우선한다.
