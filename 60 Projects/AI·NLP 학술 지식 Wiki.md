...
AI·NLP 학술 지식 Wiki는 논문을 읽은 순서대로 요약문만 쌓는 저장소가 아니다. 논문에서 반복해서 쓰이는 개념은 canonical 개념 노트로 분리하고, 여러 논문 사이의 계보는 지도에, 같은 문제를 푸는 방법의 차이는 비교에, 여러 출처를 함께 봐야 나오는 결론은 종합에 남기는 Obsidian 기반 지식 시스템이다.
