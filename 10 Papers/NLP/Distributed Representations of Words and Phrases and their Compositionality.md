...
### 선형성과 벡터 표현
Skip-gram의 선형 구조가 유추 task에 유리하게 작용한다고 생각할 수 있다. 그러나, 고도로 비선형인 [[Recurrent Neural Network]] 계열 모델도 학습 데이터가 충분히 커지면 동일 task에서 성능이 향상된다는 선행 연구가 있어, 선형성 자체가 필수 조건은 아님을 시사한다.
