...
목표 구조와 필드의 기준은 `90 Meta/Vault Schema.md`, 태그의 기준은 `90 Meta/태그 일람.md`다. `30 Maps/Comparisons/`와 `30 Maps/Syntheses/`는 각각 `35 Comparisons/`, `37 Syntheses/`로 옮길 수 있는 legacy 위치지만 자동 이동하지 않는다.
...
8. `summary`, `maturity`, `last_reviewed`는 실제 본문을 읽고 판단해야 하므로 기계적으로 채우지 않는다.
...
- 실행 전 예상 이동 수와 수정 필드를 보고하고 Dry Run 결과를 남긴다.
